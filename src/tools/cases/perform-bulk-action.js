  ...
  static getDefinition() {
    return {
      ...
      inputSchema: z.object({
        ...
        })).min(1).describe('Array of case objects to perform the action on. Each case object must contain an ID property with the full case handle.'),
        runningMode: z.enum(['async']).optional().describe('Execution mode for Launchpad only. "async" schedules the action to be performed in the background rather than immediately. Not applicable for Infinity which always executes synchronously. Note: As of Launchpad 4.3, there is no way to check the status of asynchronous bulk action processing.'),
