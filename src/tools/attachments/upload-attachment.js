  ...
  static getDefinition() {
    return {
      ...
      inputSchema: z.object({
        filePath: z.string().optional().describe('Path to file on local filesystem (preferred for desktop clients like Cline). Example: "/home/user/document.pdf" or "C:\\Users\\user\\file.txt"'),
        fileContent: z.string().optional().describe('Base64-encoded file content (for web clients or when file system access is restricted). Use this when filePath is not available.'),
        fileUrl: z.string().optional().describe('URL to file that can be fetched (http://, https://, file://, data:// schemes). Alternative when direct file access is not possible.'),
        fileName: z.string().optional().describe('Original filename with extension (required when using fileContent or fileUrl). Example: "report.pdf", "image.jpg"'),
        mimeType: z.string().optional().describe('MIME type override (auto-detected from filename/content if not provided). Example: "application/pdf", "image/jpeg"'),
        appendUniqueIdToFileName: z.boolean().optional().describe('Whether to append a unique identifier to the filename to prevent naming conflicts. Pega will add timestamp-based unique ID to filename.'),
