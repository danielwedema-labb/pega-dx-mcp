import { json_indent } from "./dx_api_constants.js";
import { to_bool, to_enum, to_lower } from "./dx_api_helper_procs.js";
import { action_t, assignment_t, component_t, component_type_strings, component_type_t, field_t, paragraph_t } from "./dx_api_model_types.js";
// Maps component type to string.
function to_c_str(type) {
    return component_type_strings[type];
}
// Returns the component type which corresponds to the provided string.
function to_component_type(str) {
    return to_enum(str, component_type_strings);
}
// Produces a debug string for the component type and name.
function to_string(type, name, ref_type) {
...
export function is_editable(component, field) {
    if (component.is_readonly)
        return false;
    if (component.is_disabled)
        return false;
    if (field?.is_special)
        return false;
    if (field?.is_class_key)
        return false;
    return true;
}
// Creates a key from a class id and a name such as: "The-Class-ID.TheName"
function make_key(class_id, name) {
    return `${class_id}.${name.split('.').slice(-1)[0]}`;
}
...
function resolve_label(raw_label, fields, class_id) {
    console.assert(!!class_id);
    let result = raw_label;
    if (raw_label) {
        if (raw_label.startsWith('@L ')) {
            // "@L Blah"
            //  0123456
            //     ^
            //     Start here.
            result = raw_label.substring(3);
        }
        else if (raw_label.startsWith('@FL ')) {
            // "@FL .Blah"
            //  012345678
            //       ^
            //       Start here.
            const field = resolve_field(raw_label.substring(5), fields, class_id);
            result = field?.label || result;
        }
    }
    return result;
}
...
function get_content(content, class_id, name, strict_mode = true) {
    let result = '';
...
//  "Blah Blah": "Blah Blah"
function resolve_name(raw_name, content, class_id, dereference_property_name = true) {
    console.assert(!!class_id);
    let result = raw_name;
    if (raw_name && raw_name.length >= 2 && raw_name.substring(0, 2) == "@P") {
        // "@P .Blah"
        //  01234567
        //      ^
        //      Start here.
...
export function validate_component_r(component, components, fields) {
    let is_valid = true;
...
function resolve_field(field_id, fields, class_id) {
    return field_id.split('.')
        .reduce((parentPage, propertyName) => fields.get([parentPage?.page_class, propertyName].join('.')), { page_class: class_id });
}
function resolve_datasource(datasource, fields, class_id) {
    if (!datasource)
        return [];
    if (typeof datasource === 'string') {
        // @ASSOCIATED .Geslacht
        // 01234567890123
        if (datasource.startsWith("@ASSOCIATED")) {
            const field = resolve_field(datasource.substring(13), fields, class_id);
            if (field) {
                const source = JSON.parse(field.json).datasource;
                return resolve_datasource(source, fields, class_id);
            }
        }
    }
...
function resolve_instructions(instructions, paragraphs) {
    ...
    if (instructions.substring(0, 2) === '@L') {
        return instructions.substring(2);
    }
    return instructions;
}
function resolve_content(content, context) {
...
function make_component_r(component_json, app, parent_class_id = "", parent_context = "") {
    ...
    switch (new_component.type) {
        ...
        case component_type_t.component_type_flow_container:
            {
                new_component.class_id = parent_class_id;
                // ...always first.
                new_component.name = resolve_name(component_json["name"] || component_json["config"].id || component_json["config"].name || component_json["type"], app.case_info.content, new_component.class_id);
        ...
        case component_type_t.component_type_view:
            {
                ...
                const config_json = component_json["config"];
                if (config_json["template"])
                    new_component.ref_type = to_component_type(config_json["template"]);
        ...
        case component_type_t.component_type_text_input:
        case component_type_t.component_type_date:
        case component_type_t.component_type_decimal:
        case component_type_t.component_type_currency:
        case component_type_t.component_type_integer:
        ...
        case component_type_t.component_type_url:
            {
                ...
                if (config_json["disabled"])
                    new_component.is_disabled = to_bool(config_json["disabled"]);
                if (config_json["readOnly"])
                    new_component.is_readonly = to_bool(config_json["readOnly"]);
                if (config_json["required"])
                    new_component.is_required = to_bool(config_json["required"]);
                if (config_json["datasource"])
                    new_component.options = resolve_datasource(config_json["datasource"], app.resources.fields, parent_class_id);
                // Always last:
    ...
    if (!new_component.name
        ...
        const config_json = component_json["config"];
        if (config_json?.value?.startsWith('@P ')) {
            const field = resolve_field(config_json.value.substring(4), app.resources.fields, new_component.class_id);
            if (field) {
                new_component.key = make_key(field.class_id, field.id);
            }
            else {
                throw new Error(`Failed to resolve field for component:\n${new_component.json}`);
            }
        }
        else {
...
export function parse_dx_response(app, response_body) {
    ...
    if (j["nextAssignmentInfo"]) {
        app.next_assignment_id = j["nextAssignmentInfo"]["ID"];
    }
    // UI resources.
    if (j["uiResources"]) {
        ...
        // Fields:
        {
            for (const field_array of Object.keys(fields_json)) {
                for (const value of fields_json[field_array]) {
                    ...
                    if (to_lower(new_field.type) == "unknown")
                        continue;
                    new_field.class_id = value["classID"];
                    if (value["pageClass"])
                        new_field.page_class = value["pageClass"];
                    if (value["label"])
                        new_field.label = value["label"];
                    if (value["isSpecial"])
                        new_field.is_special = to_bool(value["isSpecial"]);
                    if (value["isClassKey"])
                        new_field.is_class_key = to_bool(value["isClassKey"]);
