import { app_context_t, Datasource } from "./dx_api_app_types";
...
function resolve_field(field_id: string, fields: field_map_t, class_id: string): field_t {
    return field_id.split('.')
        .reduce((parentPage, propertyName) => fields.get([parentPage?.page_class, propertyName].join('.'))!,
            { page_class: class_id } as field_t
        );
}
        ...
        // Fields:
        {
            for (const field_array of Object.keys(fields_json)) {
                for (const value of fields_json[field_array]) {
                    ...
                    if (value["pageClass"]) new_field.page_class = value["pageClass"];
